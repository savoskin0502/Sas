data auto;
   input symboling losses mark $ fuelType $ aspiration $ doorsNum $ bodyStyle $ driveWheels $ engineLocation $ wheelBase length width height curbWeight engineType $ cylindersNum $ engineSize fuelSystem $ bore stroke compressionRatio horsepower peakRPM xcityMPG highwayMPG price;
datalines;
...
***********************************************************;
*  Linear regression model to predict price of the car    *;
***********************************************************;

/* Collinearity between our discrete and continuous variables*/
proc reg data=auto;
	model price = symboling losses horsepower peakRPM xcityMPG highwayMPG/vif;
run;

/* High Variance Inflation Factor => multicollinearity, del cityMPG*/
proc reg data=auto;
	model price = symboling losses horsepower peakRPM highwayMPG/vif;
run;

/* del highwayMPG: high vif */
proc reg data=auto;
	model price = symboling losses horsepower peakRPM/vif;
run;
/* Values in range (1.11 - 1.6): seems good*/
/* 1) highwayMPG&cityMPG affects on horsepower */

proc genmod data=auto;
    class mark (param=ref ref='mercedes')
    fuelType (param=ref ref='diesel')
    engineType(param=ref ref='ohc')
    cylindersNum(param=ref ref='five');
    model price = symboling losses mark fuelType engineType cylindersNum horsepower peakRPM/ corrb;
run;

/*
	1) Exclude variables with small number of observations (1 with cylindersNum 8)
	2) Build corr matrix

Conclusion:
	From results we see that:
		1)Intercept = 12595.92
		2)Symboling = -303, Each point of risk(less safety), make
			our machine cheaper, statistically significant at 10%
		3)Brand(mark): values show us on which value our machines different from mercedez,
			much expensive than mercedez is only jaguar
		4)EngineType: more->high price
		5)Horsepower: statistically significant at 1% level, change per 1u in regressor=>
			+- 86.9 dollars in price (max=288, 288*86.9 = 25027)
		6)Losses: 1 point change in regressor change our price on 11, low for the reason of
			max 256, but statistically significant at 10% level
		7)PeakRPM: not statistically significant
*/

proc means data = auto n mean var min max; 
	class mark fuelType engineType cylindersNum;
	var price losses horsepower symboling;
run;
...
proc genmod data=auto plots=all;
    class mark (param=ref ref='mercedes')
    fuelType (param=ref ref='diesel')
    engineType(param=ref ref='ohc')
    cylindersNum(param=ref ref='five')/ param=glm;
    model price = symboling losses mark fuelType engineType cylindersNum horsepower/dist=normal;
run;

proc reg data=auto;
	model price = losses height stroke peakRPM horsepower/ all;
run;

proc anova data=auto;
	class mark;
	model horsepower=mark;
run;

data test; 
  pval = 1 - probchi(162,134);
run;
...
/* 		
Final model: 
	price = 16483.13 - 295.648*Symboling + 9.241*losses + brand(0 if mercedez) +
		+ engineType(0 if ohc) + cylindersNum(0 if five) + 88.0465*horsepower;
*/
