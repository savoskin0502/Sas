...
proc corr data = BodyFat nosimple;
  var Age Weight Height
        ...
        Knee Ankle Biceps Forearm Wrist;
run;
/* From the table we see for which variables there are correlation less than 0.3,
	so let's exclude them */
/* There are the next variables: age, height*/

/* Variables with max correlation(excluding themselfes): Hip*Weight~0.94,
	Abdomen*Adioposity~0.92, Chest*Adioposity~0.91, Abdomen*Chest~0.91   */
	
/* Rerun and check that now all is ok(no variables with low correlation) */

/* Before we test the H0: coefficient correlation is zero, So H1: coefficient of correlation
	is significantly different from zero, so the answer on H1 is the next:
		all variables excluding which was with low correlation, which we choose before
		(excluding age and height)*/
proc corr data = BodyFat nosimple;
  var Weight 
        Adioposity FatFreeWt Neck Chest Abdomen Hip Thigh
        Knee Ankle Biceps Forearm Wrist;
...
ods graphics off;
title 'BodyFat graphs Analysis';
proc univariate data=BodyFat noprint;
   histogram PctBodyFat Age Weight Height
        Adioposity FatFreeWt Neck Chest Abdomen Hip Thigh
        Knee Ankle Biceps Forearm Wrist;
run;

ods trace on / label;
proc univariate data=BodyFat mu0=3.5;
   var PctBodyFat Age Weight Height
        Adioposity FatFreeWt Neck Chest Abdomen Hip Thigh
        Knee Ankle Biceps Forearm Wrist;
   output out=str probt=prd mean=meanVal;
run;
ods trace off;

ods graphics on;
proc reg data = BodyFat plots=all;
	model PctBodyFat = Weight 
        Adioposity FatFreeWt Neck Chest Abdomen Hip Thigh
        Knee Ankle Biceps Forearm Wrist/ all;
/*     output out = nb_pred predicted = pred1; */
	output out=p dffits=infl_p predicted=pred_values;
run;


/* Q1.2(answer): we can't assert that for variables with maximum correlation there are fully
	linear dependency, we know only that there is influence part in equation between them, 
	if we want to see on the model and want to proof that factors really can affect on
	the mpdel, we should find that variables using variance inflation factor to check on coll,
	FROM LRM ASSUMPTIONS WE KNOW THAT REGRESSORS MUST BE INDEPENDENT,
	let's proof my assuming excluding factors using variance inflation factor*/

/* Let's exclude 
	variables which are in linear relationships in the model */
proc reg data = BodyFat;
 	model PctBodyFat =  Weight
        Adioposity FatFreeWt Neck Chest Abdomen Hip Thigh
        Knee Ankle Biceps Forearm Wrist/vif collinoint;
run;
/* VIF value of Weight~39, exclude it */
proc reg data = BodyFat;
 	model PctBodyFat =  
        Adioposity FatFreeWt Neck Chest Abdomen Hip Thigh
        Knee Ankle Biceps Forearm Wrist/vif collinoint;
run;
/* VIF value of Abdomen~14, exclude it */
proc reg data = BodyFat;
 	model PctBodyFat =  
        Adioposity FatFreeWt Neck Chest Hip Thigh
        Knee Ankle Biceps Forearm Wrist/vif collinoint;
run;
/* VIF value of Adioposity~12, exclude it */
proc reg data = BodyFat;
 	model PctBodyFat =  
        FatFreeWt Neck Chest Hip Thigh
        Knee Ankle Biceps Forearm Wrist/vif collinoint;
run;
/* VIF value of Hip~9, exclude it */
proc reg data = BodyFat;
 	model PctBodyFat =  
        FatFreeWt Neck Chest Thigh
        Knee Ankle Biceps Forearm Wrist/vif collinoint;
run;

/* VIF value of Thigh~4.5, del */
proc reg data = BodyFat;
 	model PctBodyFat =  
        FatFreeWt Neck Chest
        Knee Ankle Biceps Forearm Wrist/vif collinoint;
run;
/* The same for Neck, delete..*/
proc reg data = BodyFat;
 	model PctBodyFat =  
        FatFreeWt Chest
        Knee Ankle Biceps Forearm Wrist/vif collinoint;
...
/* Ok, now we have model with seven prediction variables, let's build our regression */
proc reg data = BodyFat;
 	model PctBodyFat =  
        FatFreeWt Chest
        Knee Ankle Biceps Forearm Wrist;
run;
/* We see that F-statistics said us that our model is statistically significant, 
	in other words described our model fully */
/* For all predictor variables we see that there are 5 statistically significant 
	parameters and 2 non-statistically significant*/
/* Non-statistically significant paramteters: Foream(p_value~0.44) and Wrist(p_value~0.81) */
/* Statistically significant parameters: FatFreeWt(p_value~<0.0001), Chest(p_value~<0.0001).
	Knee(p_value~<0.0001), Ankle(p_value~0.0475), Biceps(p_value~<0.0001)*/
/* So the final model is: PctBodyFat = Knee Ankle Biceps Forearm Wrist, let's run */
proc reg data = BodyFat;
 	model PctBodyFat = FatFreeWt Chest Knee Ankle Biceps;
run;
/* Final equation is the next: PctBodyFat = -78.94274 -0.41046*FatFreeWt + 0.73771*Chest +
	+ 1.42538*Knee + 0.34764*Ankle + 0.60935*Biceps; */

/* Alternative of reg procedure is the next procedure: */
proc glm data=BodyFat plots=all;
	model PctBodyFat = FatFreeWt Chest Knee Ankle Biceps/ solution ss3;
run;
/* From the results we see the next: our model became more exact that in the case before
	(proc reg), values for the estimate parameters are more exact, 
	but we can said that result's are the same*/
	
/* Let's create dataset and model without regressors which are not needed*/
proc sql;
	create table model2 as
	Select PctBodyFat,FatFreeWt,Chest,Knee, Ankle,Biceps From BodyFat;
run;

/* Ok, now we will make prediction based on the type 1,2,3,4 Root Mean Square Errors,
	we will use glm procedure with ss parameters*/
proc glm data=BodyFat plots=all;
	model PctBodyFat = FatFreeWt Chest Knee Ankle Biceps/ solution ss3 ss1 ss2 ss4 ;
run;
/* Root MSE value is low, which can indicate about also good r */

/* About the best model: when we choose between models, we should understand that 
	to identify the best model we must hold the LRM assumptions, 
	assumptions about collinearity, variables independency(in the regressors), etc
	Also we shoudl use model with only needed variables excluded such parameters, which
	not really describe our model. Exclude non-stat significant variables and have better model
	Less discharge - better, our model will be more near about the really mean
	*/
/* For additional testing in our model we can estimate by F-test is really parameter needed or not */
/* Using estimations: */

proc glm data=BodyFat plots=all;
	model PctBodyFat = FatFreeWt Chest Knee Ankle Biceps ;
	estimate 'Fat Yes vs No' FatFreeWt 1 0;
	estimate 'Chest Yes vs No' Chest 1 0;
	estimate 'Knee Yes vs No' Knee 1 0;
	estimate 'Ankle Yes vs No' Ankle 1 0;
	estimate 'Biceps Yes vs No' Biceps 1 0;
run;
/* All ok, go next, no variables which need to exclude */

/* Assumptions:
	No collinearity - R_square more, but it's not really true;
	Homoscedasticity: we don't check it before, use cook's test and check it after
	Error term is ~0
	Variance~(0,Q^2)
	Mean((0,Q^2))
	*/

/* Now, let's find discharging using cook's test */

proc reg data= BodyFat plots=cooksd;
	model PctBodyFat = FatFreeWt Chest Knee Ankle Biceps ;
run;
/* From cook's D statistic there are 16 variables in the model, 
	which will be better to delete*/
proc sql;
	create table model3 as
	Select * from model2 Where PctBodyFat<=25;
run;
/* Must problematic: which was strong correlated, vif was more,
	no true F-statistic, t-statistic */

/* Conclusion is the next: the best model will be the last model with 5 predictors,
	RMSE metric became less(less discharging), t-statistic and f statistic is real for
	the reason of LRM true assumptions*/
