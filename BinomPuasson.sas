data pr;
   input iden nu const z1 $ z2 z3 z5 $ z4 z6 nus z7 z8 yPT nuse nused;
datalines;
...
proc means data =pr n mean var min max; 
  var yPT z2 z3 z4 z6 z7 z8;
run; 
   
proc freq data=pr;
	tables yPT / plots=freqplot;
run;

proc means data = pr mean var;
  class z1 z5;
  var yPT;
run;

proc genmod data = pr;
	class z1 z5 / param=glm;
	model yPT = z1 z2 z3 z4 z5 z6 z7 z8 / dist=poisson;
	store p1;
run;

proc genmod data = pr;
	class z1 z5 / (param=ref ref =first);
	model yPT = z1 z2 z3 z4 z5 z6 z7 z8 / dist=poisson;
	store p1;
run;

data pvalue;
  df = 591; chisq=4121.0053;
...
proc univariate data = pr noprint;
  histogram yPT / midpoints = 0 to 20 by 1 vscale=count;
run;
...
proc means mean var n data = pr;
  by z1 z5;
  var yPT;
run;

proc genmod data = pr;
  class z1 (param=ref ref=first) z5 (param=ref ref=first);
  model yPT = z1 z2 z3 z4 z5 z6 z7 z8/ type3 dist=negbin;
run;

data test; 
  pval = 1 - probchi(545.7691, 591);
run;
proc nlmixed data = pr;
 parms b0 = 0 b1 = 0 b2 = 0 b3 = 0 b4 = 0 b5 = 0 b6 = 0 b7 = 0 b8 = 0;
 mu = exp(b0 + b1 * z1 + b2 * z2 + b3 * z3 + b4 * z4 +
 b5 * z5 + b6 * z6 + b7 * z7 + b8*z8);
 ll = -mu + yPT * log(mu) - log(fact(HOSP));
 model yPT ~ general(ll);
